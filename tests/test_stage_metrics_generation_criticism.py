...
# Stubs to satisfy imaginator_flow imports if needed in this isolated run
...
from imaginator_flow import RUN_METRICS, run_criticism_async, run_generation_async
...
def test_generation_stage_metrics_recorded():
    _reset_metrics()
    ...
    with patch("imaginator_flow.call_llm_async", new=AsyncMock(side_effect=fake_llm)):
        res = asyncio.run(
            run_generation_async(
                analysis_json={"skills": ["Python"], "seniority": "mid"},
                job_ad="Backend"
            )
        )
...
def test_criticism_stage_metrics_recorded():
    _reset_metrics()

    async def fake_llm(*args, **kwargs):
        return "{\"score\":0.8,\"feedback\":\"Strong alignment\"}"

    with patch("imaginator_flow.call_llm_async", new=AsyncMock(side_effect=fake_llm)):
        res = asyncio.run(
            run_criticism_async(
                generated_text="Entry",
                job_ad="Senior Backend"
            )
        )

    assert "score" in res or isinstance(res, str)
