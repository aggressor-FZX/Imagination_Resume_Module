...
# Detect if we're using PUMS endpoint (Ithaca) vs legacy Tesseract
_IS_PUMS_ENDPOINT = "pums" in DATAUSA_API_BASE.lower()

# PUMS uses different cube names than legacy ACS/BLS cubes
if _IS_PUMS_ENDPOINT:
    # PUMS endpoint uses pums_5 cube for all occupation data
    DATA_USA_CUBE_WAGE = os.getenv("DATA_USA_CUBE_WAGE", "pums_5")
    DATA_USA_CUBE_WORKFORCE = os.getenv("DATA_USA_CUBE_WORKFORCE", "pums_5")
    DATA_USA_CUBE_GROWTH = os.getenv("DATA_USA_CUBE_GROWTH", "")  # PUMS doesn't have BLS growth data
else:
    # Legacy Tesseract cubes
...
class CareerProgressionEnricher:
    ...
    # US State mapping: abbreviation -> [full_name, abbrev, common_variants]
    _STATE_PATTERNS: Dict[str, List[str]] = {
        ...
        "WI": ["WISCONSIN", "WI"],
        "WY": ["WYOMING", "WY"],
        "DC": ["DISTRICT OF COLUMBIA", "DC", "WASHINGTON DC", "WASHINGTON D.C."],
    }

    @classmethod
    def _parse_state_from_location(cls, location: str) -> str:
        """
        Derive state code from location string for CareerOneStop (state-level API).
        
        Handles formats like:
        - "Bellevue, Washington, US"
        - "Seattle, WA"
        - "Seattle, WA 98101"
        - "Washington" (ambiguous, defaults to WA state)
        - "Remote / WA"
        
        Returns 2-letter state code or "US" if no match found.
        """
        if not location or not isinstance(location, str):
            return "US"
        
        # Normalize: uppercase, strip extra whitespace
        loc = re.sub(r'\s+', ' ', location.strip()).upper()
        
        # Remove ZIP codes (5 digits or 5+4 format) to avoid matching numbers as states
        loc = re.sub(r'\b\d{5}(-\d{4})?\b', '', loc)
        
        # Remove common suffixes like "US", "USA", "UNITED STATES"
        loc = re.sub(r',?\s*(US|USA|UNITED STATES)\b', '', loc)
        
        # Strategy 1: Look for comma-separated state (e.g., "Seattle, WA")
        # Match 2-letter state code after comma
        comma_match = re.search(r',\s*([A-Z]{2})\b', loc)
        if comma_match:
            state_code = comma_match.group(1)
            if state_code in cls._STATE_PATTERNS:
                return state_code
        
        # Strategy 2: Look for full state name anywhere in string
        for state_code, patterns in cls._STATE_PATTERNS.items():
            for pattern in patterns:
                # Use word boundaries for full names to avoid partial matches
                # e.g., "New York" should not match "York" in "Yorktown"
                if len(pattern) > 2:
                    if re.search(r'\b' + re.escape(pattern) + r'\b', loc):
                        return state_code
                else:
                    # For 2-letter codes, require word boundaries
                    if re.search(r'\b' + re.escape(pattern) + r'\b', loc):
                        return state_code
        
        # Strategy 3: Check for state code standalone (e.g., "Remote / WA")
        for state_code in cls._STATE_PATTERNS.keys():
            if re.search(r'\b' + state_code + r'\b', loc):
                return state_code
        
        # No match found - return "US" for national data instead of assuming WA
        print(f"⚠️  [CareerOneStop] Could not parse state from location: '{location}', defaulting to national data")
        return "US"

    def _get_cos_wages(self, keyword: str, state: str = "WA") -> Dict[str, Any]:
        """
        Get wages from CareerOneStop Occupation Detail API.
        keyword: O*NET code (e.g. 15-1243.00) or job title.
        state: 2-letter state code (e.g. "WA", "CA"). If "US", returns empty (no national endpoint).
        Returns median / pct25 / pct75 when available; empty dict otherwise.
        """
        if not (CAREERONESTOP_USER_ID and CAREERONESTOP_TOKEN and keyword):
            return {}
        
        # CareerOneStop occupation API requires a state; "US" means we couldn't parse one
        if state == "US":
            logger.debug("[CareerOneStop] Skipping state-specific wage call for national/unknown location")
            return {}
        try:
            url = f"{CAREERONESTOP_BASE_URL}/occupation/{CAREERONESTOP_USER_ID}/{keyword}/{state}/0"
            headers = {
            ...
            data = resp.json()
            # API can return OccupationDetail or OccupationMatchResult
            details = (
                data.get("OccupationDetail")
                or data.get("occupationdetail")
                or data.get("OccupationMatchResult")
                or []
            )
            if not details:
                return {}
            detail = details[0] if isinstance(details, list) else details
            wages = detail.get("Wages", {}) or detail.get("wages", {})
            national = wages.get("NationalWagesList", []) or wages.get("nationalWagesList", [])
            ...
            median_entry = next(
                (
                    ...
                    and (str(w.get("RateType", "")).lower() == "annual")
                    and ("median" in str(w.get("WageLevel", "")).lower())
                ),
                None,
            )
            if not median_entry:
                return {}
            return {
                "median": median_entry.get("Median"),
                "pct25": median_entry.get("Pct25"),
                "pct75": median_entry.get("Pct75"),
            }
        except Exception as e:
            print(f"⚠️  CareerOneStop wages failed: {e}")
            return {}
    ...
    def get_full_career_insights(
        ...
        # When Data USA has no wage data, fall back to CareerOneStop (state-level)
        # Try increasingly broad keywords to maximize chance of COS coverage
        if workforce_data.get("data_source") == "Data unavailable" and not workforce_data.get("average_wage"):
            state = self._parse_state_from_location(location)
            
            # Build fallback keyword chain: specific → broad tech fallback
            cos_keywords = [
                onet_code,           # "15-1243.00" - specific O*NET code
                job_title,           # "AI Data Engineer" - user's job title
            ]
            
            # Add stripped/variant forms if job_title contains qualifiers
            job_clean = (job_title or "").strip()
            if job_clean:
                # Add "Data Engineer" variant if title has "AI Data Engineer"
                if "data engineer" in job_clean.lower() and "data engineer" not in [k.lower() for k in cos_keywords if k]:
                    cos_keywords.append("Data Engineer")
                # Add "Software Developer" as broad tech fallback - COS always has this
                if "software developer" not in [k.lower() for k in cos_keywords if k]:
                    cos_keywords.append("Software Developer")
            
            # Try each keyword until we get wages
            cos_wages = {}
            used_keyword = None
            for keyword in cos_keywords:
                if not keyword:
                    continue
                cos_wages = self._get_cos_wages(keyword, state)
                if cos_wages.get("median") is not None:
                    used_keyword = keyword
                    break
            
            if cos_wages.get("median") is not None:
                workforce_data["average_wage"] = int(cos_wages["median"])
                workforce_data["data_source"] = f"CareerOneStop ({state}) via '{used_keyword}'"
                if cos_wages.get("pct25") is not None:
                    workforce_data["pct25"] = int(cos_wages["pct25"])
                if cos_wages.get("pct75") is not None:
                    workforce_data["pct75"] = int(cos_wages["pct75"])
                print(f"   Using CareerOneStop wages for {state} via '{used_keyword}': median ${workforce_data['average_wage']:,}")
    ...
    def _get_cube_schema(self, cube: str) -> Dict[str, Any]:
        ...
        response = httpx.get(url, headers={"User-Agent": DATAUSA_USER_AGENT}, timeout=20)
        response.raise_for_status()
        data = response.json()
    ...
    def _get_members(self, cube: str, level: str) -> List[Dict[str, Any]]:
        ...
        response = httpx.get(url, headers={"User-Agent": DATAUSA_USER_AGENT}, timeout=25)
        response.raise_for_status()
        data = response.json()
    ...
    def _query_tesseract(
        ...
        response = httpx.get(
            DATAUSA_API_BASE,
            params=params,
            headers={"User-Agent": DATAUSA_USER_AGENT},
            timeout=25,
        )
        response.raise_for_status()
        return response.json()
    ...
    def _get_workforce_trends(
        ...
        try:
            ...
            # Try PUMS direct query first if using PUMS endpoint
            if _IS_PUMS_ENDPOINT:
                pums_data = self._query_pums_direct(soc_code, geo_id)
                if pums_data.get("average_wage"):
                    return pums_data
                # PUMS failed or returned empty, fall through to empty data
                return self._empty_workforce_data()
            
            # Legacy Tesseract path (for non-PUMS endpoints)
    ...
    def _query_pums_direct(
        self,
        soc_code: str,
        geo_id: Optional[str] = None,
    ) -> Dict[str, Any]:
        """
        Query PUMS (Public Use Microdata Sample) API directly.
        
        PUMS uses a different query format than legacy Tesseract:
        - cube: always "pums_5"
        - include: "Workforce Status:true;Nation:01000US;Detailed Occupation:{soc}"
        - measures: "Average Wage,Average Wage Appx MOE,Record Count"
        - drilldowns: "Year"
        - filters: "Record Count.gte.5"
        
        Returns wage and workforce data or empty dict on failure.
        """
        if not _IS_PUMS_ENDPOINT:
            return {}
        
        try:
            # Build the include filter
            if geo_id and isinstance(geo_id, str) and geo_id.startswith("04000US"):
                geo_filter = f"State:{geo_id}"
            else:
                geo_filter = "Nation:01000US"
            
            include_filter = f"Workforce Status:true;{geo_filter};Detailed Occupation:{soc_code}"
            ...
            print(f"   Querying PUMS for SOC {soc_code}...")
            ...
            rows = sorted(rows, key=lambda x: int(x.get("Year", 0)), reverse=True)
            
            latest = rows[0]
            prev = rows[1] if len(rows) > 1 else None
            
            avg_wage = latest.get("Average Wage")
            prev_wage = prev.get("Average Wage") if prev else None
            record_count = latest.get("Record Count")
            
            # Calculate growth if we have previous data
            wage_growth = 0
            if avg_wage and prev_wage:
                wage_growth = ((avg_wage - prev_wage) / prev_wage) * 100
            
            return {
                "workforce_size": int(record_count) if record_count else None,
                "average_wage": int(avg_wage) if avg_wage else None,
                "yoy_growth_workforce": None,  # PUMS doesn't have workforce growth directly
                "yoy_growth_wage": round(wage_growth, 1) if wage_growth else 0,
                "workforce_delta_3yr": None,
                "year_latest": latest.get("Year"),
                "year_previous": prev.get("Year") if prev else None,
                "time_series": rows[:5],
                "data_source": f"Data USA PUMS ({geo_filter})",
            }
        except Exception as e:
            print(f"   PUMS query failed: {e}")
            return {}
    ...
    def calculate_market_intel(
        ...
        """
        # If we have no usable workforce data, avoid misclassifying as "Market Saturation"
        if not workforce_data or workforce_data.get("data_source") == "Data unavailable":
            status_label = "Data unavailable"
            demand_label = "Unknown"
            has_bright_outlook = False
            bright_outlook_tags: List[str] = []
            is_shortage = False
            workforce_growth_pct = 0.0
            wage_growth = 0.0
            # Still pass through average_wage if COS filled it later (see branch below)
            ...
            return {
                ...
                "average_wage": workforce_data.get("average_wage") if workforce_data else None,
                "workforce_size": workforce_data.get("workforce_size") if workforce_data else None,
                "data_year": workforce_data.get("year_latest") or workforce_data.get("year", "Unknown") if workforce_data else "Unknown",
                "narrative": narrative,
            }
        # CareerOneStop-only: we have wages but no growth data — don't label as "Market Saturation"
        if workforce_data.get("data_source", "").startswith("CareerOneStop"):
            status_label = workforce_data["data_source"]
            demand_label = "Unknown"
            has_bright_outlook = False
            bright_outlook_tags = []
            is_shortage = False
            ...
            if workforce_data.get("pct25") is not None:
                out["pct25"] = workforce_data["pct25"]
            if workforce_data.get("pct75") is not None:
                out["pct75"] = workforce_data["pct75"]
            return out
        # PUMS-only: we have wages and wage growth but no workforce growth metrics
        # Don't classify as "Market Saturation" - use wage growth instead
        if workforce_data.get("data_source", "").startswith("Data USA PUMS"):
            wage_growth = workforce_data.get("yoy_growth_wage", 0) or 0
            
            # Use wage growth as proxy for demand (high wage growth = high demand)
            if wage_growth > 7:
                demand_label = "High Demand"
            elif wage_growth > 2:
                demand_label = "Stable Market"
            else:
                demand_label = "Moderate Growth"
            
            status_label = f"Data USA PUMS ({workforce_data.get('year_latest', 'latest')})"
            has_bright_outlook = False
            bright_outlook_tags = []
            is_shortage = False
        ...
        # Extract workforce growth metrics when data is available
