#!/usr/bin/env python3
"""
Comprehensive test using real resume samples and job ads
Tests narrative detection fix with actual data
"""
import asyncio
import sys
import os
...
async def test_resume_job_combination(resume_text, job_ad, test_name):
    """Test a resume/job ad combination"""
    print(f"\n{'='*70}")
    ...
    try:
        ...
        if final_section:
            if check_for_narrative(final_section):
                has_narrative = True
                narrative_found_in.append("final_written_section")
        
        if final_markdown:
            if check_for_narrative(final_markdown):
                has_narrative = True
        ...
        if has_narrative:
            ...
            resume_indicators = ["•", "-", "Developed", "Built", "Implemented", "Led", "Created", "Reduced", "Increased"]
            has_resume_format = any(ind in final_section for ind in resume_indicators)
            
            if has_resume_format:
                print(f"   ✅ Contains resume format indicators (bullets, action verbs)")
            else:
...
async def run_all_tests():
    ...
    print("🚀 Starting Comprehensive Narrative Detection Tests")
    print("=" * 70)
    ...
    # Test 3: Data Scientist + Senior Software Engineer (mismatch test)
    if "Data Scientist" in resume_samples and "Senior Software Engineer" in job_ads:
        result = await test_resume_job_combination(
            resume_samples["Data Scientist"],
            job_ads["Senior Software Engineer"],
            "Data Scientist Resume → Senior Software Engineer Job (Mismatch)"
        )
        test_results.append(("Data Scientist → Senior Software Engineer", result))
