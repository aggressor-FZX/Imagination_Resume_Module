...
class PipelineOrchestrator:
    ...
    async def run_pipeline(
        ...
        **kwargs,
        ...
        Args:
            ...
            creativity_mode: Creativity preset for generation
            extracted_job_title: Extracted or user-provided job title
            ...
            classified_skills: Structured skills from FastSVM/Hermes
            job_titles: Matching job titles
            **kwargs: Additional parameters (education, projects, certifications)
        ...
        logger.info(
            f"[ORCHESTRATOR] Starting pipeline for resume ({len(resume_text)} chars), "
        ...
        try:
            ...
            # Check if projects should be merged with experiences (for students/career changers)
            # Projects are passed via kwargs from the backend
            projects = kwargs.get("projects", [])
            education = kwargs.get("education", [])

            if projects:
                logger.info(
                    f"[ORCHESTRATOR] Found {len(projects)} projects to potentially merge with experiences"
                )
                ...
                for proj in projects[:3]:  # Limit projects
                    if isinstance(proj, dict):
                        ...
                        if any(exp.get("company") == proj_name for exp in experiences):
                            continue
                        ...
                        project_exp = {
                            ...
                            "is_project": True,  # Flag for downstream processing
                        }
                        experiences.append(project_exp)

            if education:
                logger.info(f"[ORCHESTRATOR] Processing {len(education)} education entries for context")
                for edu in education:
                    if isinstance(edu, dict):
                        # Treat relevant coursework/theses as pseudo-experience for researchers
                        edu_exp = {
                            "company": edu.get("institution", "University"),
                            "role": f"Student ({edu.get('degree', 'Degree')})",
                            "duration": edu.get("dates", ""),
                            "bullets": [f"Relevant Coursework: {', '.join(edu.get('coursework', []))}" ] if edu.get("coursework") else [],
                            "is_education": True
                        }
                        experiences.append(edu_exp)

            logger.info(
                f"[ORCHESTRATOR] Total experiences after enrichment: {len(experiences)}"
            )
            ...
            try:
                ...
                if "error" in research_data:
                    logger.error(
                        f"[ORCHESTRATOR] Stage 1 returned error: {research_data['error']}"
                    )
                    result["errors"].append(
                        f"Researcher Error: {research_data['error']}"
                    )
            ...
            gc.collect()
            await asyncio.sleep(1.5)  # 1.5s cooldown — memory peaks right after a stage returns
            ...
            gc.collect()
            await asyncio.sleep(1.5)  # 1.5s cooldown
            ...
            try:
                ...
                logger.info(
                    f"[ORCHESTRATOR] StarEditor completed. Model: {editor_data.get('model_used', 'unknown')}"
                )
            except Exception as e:
            ...
            try:
                seniority_result = self.seniority_detector.detect_seniority(
                    experiences, all_skills_for_seniority
                )
                logger.info(
                    f"[ORCHESTRATOR] Detected seniority: {seniority_result.get('level')} ({seniority_result.get('confidence'):.2f})"
                )
            except Exception as e:
            ...
            processed_skills = {
                "high_confidence": list(
                    resume_skills
                ),  # User's actual skills from resume
                "medium_confidence": [],
                "low_confidence": [],
                "inferred_skills": job_implied_skills,  # Job ad requirements (NOT user's confirmed skills)
            }
            logger.info(
                f"[ORCHESTRATOR] Skills - Resume: {len(resume_skills)}, Job Implied: {len(job_implied_skills)}"
            )
            ...
            if draft_data.get("fallback") or editor_data.get("fallback"):
                ...
                errors.append(
                    "Pipeline used fallback generation for one or more stages"
                )
            ...
            result.update(
                {
                    ...
                    "final_output": {
                        "final_written_section_markdown": editor_data.get(
                            "final_markdown", ""
                        ),
                        "final_written_section": editor_data.get(
                            "final_plain_text", ""
                        ),
                        ...
                        "quantification_analysis": editor_data.get(
    ...
    def _create_emergency_output(
        self, experiences: List[Dict], job_ad: str
    ) -> Dict[str, Any]:
        """
        ...
        for exp in experiences[:6]:  # Increased fallback coverage
            ...
            if company and role:
                ...
                markdown_lines.append(
                    "- Applied relevant skills to achieve business objectives"
                )
                markdown_lines.append(
                    "- Collaborated with team members to deliver results"
                )
    ...
    def validate_inputs(
        self, resume_text: str, job_ad: str, experiences: List[Dict]
    ) -> List[str]:
        """
