...
)  # Use STAR editor for quick ATS scoring
...
ONET_API_AUTH = os.getenv(
    "ONET_API_AUTH", "Y29naXRvbWV0cmljOjI4Mzd5cGQ="
...
CAREERONESTOP_BASE_URL = os.getenv(
    "CAREERONESTOP_BASE_URL", "https://api.careeronestop.org/v1"
)
DATAUSA_API_BASE = os.getenv(
    "DATAUSA_API_BASE", "https://api-ithaca.datausa.io/calcs/pums.jsonrecords"
)
...
try:
    ...
    logger.warning(
        "[NEW_PIPELINE] career_alchemy module not available, Career Alchemy feature disabled"
    )
            ...
            if duration:
                ...
                if years_match:
                    ...
                    months_match = re.search(
                        r"(\d+)\s*(?:month|mo)", duration, re.IGNORECASE
                    )
                    if months_match:
...
def _build_onet_search_variants(job_title: str) -> List[str]:
    ...
    for prefix in ["ai", "ml", "llm", "nlp", "gen-ai", "generative", "prompt"]:
        stripped = re.sub(
            rf"^\s*{prefix}\s+", "", clean_title, flags=re.IGNORECASE
        ).strip()
...
def _score_keyword_candidate(job_title: str, candidate_title: str, query: str) -> float:
    ...
    query_overlap = (
        len(query_tokens & candidate_tokens) / float(len(query_tokens))
        if query_tokens
        else 0.0
    )
    ...
    if lowered_title in lowered_candidate:
        score += 0.2
    if any(
        token in lowered_candidate for token in ["security", "compliance", "auditor"]
    ) and not any(
...
async def _llm_resolve_onet(
    job_title: str, llm_client: Optional[LLMClientAdapter]
) -> Dict[str, Any]:
    """
...
""".strip()
    try:
        ...
        logger.warning(
            "[NEW_PIPELINE] LLM O*NET resolution failed for '%s': %s",
            job_title,
            llm_error,
        )
        return {}
...
async def _resolve_onet_code(
    job_title: str,
    llm_client: Optional[LLMClientAdapter] = None,
    upstream_onet_code: Optional[str] = None,
) -> Dict[str, Any]:
    """
    Resolution chain:
    1) If upstream_onet_code is provided and valid, return it directly (passthrough).
    2) LLM semantic mapping + code validation
    3) Scored keyword variant search
    """
    # Passthrough: if upstream code is present and looks valid, use it
    if upstream_onet_code:
        clean_code = (upstream_onet_code or "").strip()
        # Regex check for safety before network call
        if re.fullmatch(r"\d{2}-\d{4}\.\d{2}", clean_code):
            logger.info(
                "[NEW_PIPELINE] O*NET code %s provided upstream -> using as passthrough",
                clean_code,
            )
            return {"code": clean_code, "source": "upstream_passthrough"}
        logger.warning(
            "[NEW_PIPELINE] Upstream O*NET code '%s' invalid; will resolve",
            upstream_onet_code,
        )
...
def _get_domain_fallback_occupation(
    job_title: str, inferred_domain: str = ""
) -> Dict[str, str]:
    """
    ...
    fallback_by_domain = {
        ...
        "prompt_engineering": {
            "code": "15-1299.08",
            "title": "Computer Systems Engineers",
        },
        "product_management": {
            "code": "11-3021.00",
    ...
    if any(
        ...
        for token in ["data engineer", "etl", "pipeline", "database"]
    ):
    ...
    if any(
        token in title_lower for token in ["prompt engineer", "prompt", "llm engineer"]
    ):
...
def _extract_structured_sections_from_resume(
            ...
            parsed["certifications"].append(
                {
                    ...
                    "issuer": "",
                    "year": (
                        year_regex.search(line).group(0)
                        if year_regex.search(line)
                        else ""
                    ),
                }
            )
    ...
    for key in ("education", "projects", "certifications"):
        parsed[key] = [
            item for item in parsed[key] if any(str(v).strip() for v in item.values())
        ]
...
async def run_new_pipeline_async(
    ...
    classified_skills: Optional[Dict[str, Any]] = None,
    ...
    **kwargs,
    ...
    Args:
        ...
        golden_bullets: Style reference bullets from Pinecone
        onet_code: Upstream O*NET occupation code
        experience: Structured work experience from profile
        skills: Flat list of skills from user profile
        classified_skills: Structured skills already classified by FastSVM/Hermes
        job_titles: List of possible matching job titles for the user
    ...
    if golden_bullets:
        logger.info(f"[NEW_PIPELINE] Received {len(golden_bullets)} golden bullets")
    if onet_code:
        logger.info(f"[NEW_PIPELINE] Received upstream O*NET code: {onet_code}")
    if experience:
        logger.info(f"[NEW_PIPELINE] Received {len(experience)} structured experience entries")
    if skills:
        logger.info(f"[NEW_PIPELINE] Received {len(skills)} flat skills")
    if classified_skills:
        logger.info("[NEW_PIPELINE] Received classified skills")
    if job_titles:
        logger.info(f"[NEW_PIPELINE] Received {len(job_titles)} matching job titles")
    ...
    try:
        ...
        projects_input = (
            provided_projects if isinstance(provided_projects, list) else []
        )
        education_input = (
            provided_education if isinstance(provided_education, list) else []
        )
        certifications_input = (
            provided_certifications if isinstance(provided_certifications, list) else []
        )
        ...
        effective_certifications = certifications_input or inferred_sections.get(
            "certifications", []
        )
        ...
        logger.info(
            ...
            (
                "payload"
                if projects_input
                else ("resume_text" if effective_projects else "none")
            ),
            len(effective_education),
            (
                "payload"
                if education_input
                else ("resume_text" if effective_education else "none")
            ),
            len(effective_certifications),
            (
                "payload"
                if certifications_input
                else ("resume_text" if effective_certifications else "none")
            ),
        )
        ...
        llm_client = LLMClientAdapter(
            ...
            fallback_keys=(
                openrouter_api_keys[1:] if len(openrouter_api_keys) > 1 else []
            ),
        )
        ...
        if extracted_job_title:
            logger.info(
                f"[NEW_PIPELINE] Using user-provided job title: {extracted_job_title}"
            )
        else:
            # Extract Job Title (Gemini Flash Lite) - High Priority for Frontend Display
            logger.info(
                "[NEW_PIPELINE] Extracting job title via google/gemini-2.5-flash-lite (backup: openai/gpt-4o-mini)..."
            )
            try:
                ...
                if title_response_clean:
                    ...
                    try:
                        ...
                        json_match = re.search(
                            r"\{.*\}", title_response_clean, re.DOTALL
                        )
                        if json_match:
                            try:
                        ...
                        if not title_data:
                            title_data = {
                                "job_title": title_response_clean.strip().strip('"')
                            }
                else:
                    logger.warning(
                        "[NEW_PIPELINE] Job title extraction returned an empty response"
                    )
                ...
                if extracted_job_title:
                    ...
                    logger.info(
                        f"[NEW_PIPELINE] Extracted Job Title: {extracted_job_title}"
                    )
        ...
        result = await orchestrator.run_pipeline(
            ...
            location=location or kwargs.get("location", ""),  # Pass location
            golden_bullets=golden_bullets or kwargs.get("golden_bullets"),  # Pass Golden Bullets
            onet_code=onet_code or kwargs.get("onet_code"),  # Pass O*NET code
            skills=skills,
...
Rate the alignment (0.0-1.0):"""
        ...
        if errors:
            logger.error(
                f"[NEW_PIPELINE] Pipeline encountered {len(errors)} errors: {errors}"
            )
        ...
        if extracted_skills_json:
            if isinstance(extracted_skills_json, list):
                ...
                isinstance(extracted_skills_json, dict)
                and "skills" in extracted_skills_json
            ):
        ...
        logger.info(
            f"[NEW_PIPELINE] Aggregated {len(aggregate_skills)} skills from all sources"
        )
        ...
        if domain_insights_json and isinstance(domain_insights_json, dict):
            ...
            domain_insights = {
                ...
                "skill_gap_priority": (
                    insider_tips
                    if insider_tips
                    else "Focus on demonstrating quantifiable achievements"
                ),
            }
        ...
        if not domain_insights.get("top_skills"):
            domain_insights["top_skills"] = (
                domain_vocab[:5]
                if domain_vocab
                else aggregate_skills[:5] if aggregate_skills else []
            )
        ...
        if implied_metrics_normalized:
            ...
            metrics_str = ", ".join(
                str(item) for item in implied_metrics_normalized if item
            )
            insights_list.append(f"Key metrics to target: {metrics_str}")
        if not insights_list:
            insights_list.append(
                "Focus on quantifiable achievements and relevant technologies."
            )
        ...
        onet_code = None
        if (
            not job_title_for_market
            and domain_insights_json
            and isinstance(domain_insights_json, dict)
        ):
        ...
        if location and (job_title_for_market or onet_code):
            try:
                ...
                safe_market_fallback = (
                    os.getenv("IMAGINATOR_SAFE_MARKET_FALLBACK_ENABLED", "true").lower()
                    == "true"
                )
                ...
                if not geo_id:
                    logger.warning(
                        f"[NEW_PIPELINE] No geo ID found for location: {location}, using national data"
                    )
                ...
                if not onet_code and job_title_for_market:
                    resolved_match = await _resolve_onet_code(
                        job_title_for_market,
                        llm_client,
                        upstream_onet_code=kwargs.get("onet_code"),
                    )
                    if resolved_match.get("code"):
                        onet_code = resolved_match["code"]
                        domain_insights["onet"] = {
                            "title": resolved_match.get("title")
                            or job_title_for_market,
                            "code": onet_code,
                            "status": "resolved",
                            "resolution_source": resolved_match.get(
                                "source", "unknown"
                            ),
                ...
                if onet_code:
                    try:
                        ...
                        logger.warning(
                            f"[NEW_PIPELINE] Failed to get O*NET summary for {onet_code}: {e}"
                        )
                        onet_summary = None
                ...
                try:
                    ...
                    logger.warning(
                        f"[NEW_PIPELINE] Fallback to resume-inferred: {fallback_reason}"
                    )
                    # Resume fallback
                    ...
                    if safe_market_fallback:
                        ...
                        if _is_missing_or_weak(domain_insights.get("market_demand")):
                            domain_insights["market_demand"] = (
                                "High"  # Tech roles in WA are not "Market Saturation"
                            )
                    else:
                        domain_insights["salary_range"] = _get_fallback_salary(
                            desired_job_title or extracted_job_title or "",
                ...
                logger.error(
                    f"[NEW_PIPELINE] Failed to enrich domain insights with market intel: {e}",
                    exc_info=True,
                )
        elif location:
            logger.warning(
                f"[NEW_PIPELINE] Location provided ({location}) but no job title extracted, skipping market intel"
            )
        ...
        section_completeness = {
            ...
            "has_education": bool(source_education)
            and (
                "education" in rewritten_lower
                or "b.s." in rewritten_lower
                or "degree" in rewritten_lower
            ),
            "has_projects": bool(source_projects) and ("project" in rewritten_lower),
            "has_certifications": bool(source_certifications)
            and ("certif" in rewritten_lower or "comptia" in rewritten_lower),
            "missing_sections": [],
        }
        ...
        if CAREER_ALCHEMY_AVAILABLE:
            try:
                ...
                characteristics = {
                    "canonical_title": extracted_job_title
                    or domain_insights.get("domain", "Professional"),
                    ...
                    "seniority": {
                        "level": final_output.get("seniority_level", "mid"),
                        "job_zone": (
                            seniority_analysis.get("job_zone", "4")
                            if isinstance(seniority_analysis, dict)
                            else "4"
                        ),
                        "experience_required": (
                            seniority_analysis.get("experience_required", "5-10 years")
                            if isinstance(seniority_analysis, dict)
                            else "5-10 years"
                        ),
                    },
                    ...
                    or domain_insights.get("education", []),
                    "achievements": [
                        exp.get("snippet", "")[:100]
                        for exp in experiences[:5]
                        if isinstance(exp, dict) and exp.get("snippet")
                    ],
                }
                ...
                try:
                    ...
                    if not _validate_onet_code(target_onet):
                        ...
                        logger.info(
                            f"[NEW_PIPELINE] Generating Pivot Analysis for Alchemy using code: {target_onet}"
                        )
                        try:
                            pivot_data = enricher.generate_career_pivot_analysis(
                                current_onet_code=target_onet,
                                current_job_title=extracted_job_title
                                or "Software Engineer",
                                resume_skills=[
                                    s if isinstance(s, str) else s.get("skill", "")
                            ...
                            logger.warning(
                                f"[NEW_PIPELINE] Pivot analysis failed: {pivot_err}"
                            )
                            pivot_data = None
                except Exception as pivot_err:
                    logger.warning(
                        f"[NEW_PIPELINE] Alchemy pivot enrichment failed: {pivot_err}"
                    )
                    pivot_data = None
                ...
                try:
                    ...
                    logger.warning(
                        f"[NEW_PIPELINE] Career Alchemy generation failed: {e}"
                    )
                    career_alchemy_data = None
        ...
        logger.info(
            f"[NEW_PIPELINE] Completed successfully. Status: {response['pipeline_status']}, Critique Score: {critique_score}, Tokens: {llm_usage.get('total_tokens', 0)}"
        )
        return response
