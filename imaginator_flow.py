...
def parse_experiences(text: str) -> List[Dict]:
    ...
    for exp in experiences:
        ...
        location_match = re.search(
            r'([A-Za-z\s]+,\s*[A-Z]{2})(?:\s|$|\d)',  # "City, ST" format
            title_line + " " + body
        )
        if location_match:
            ...
            location_match = re.search(
                r'([A-Za-z\s]+,\s*[A-Za-z\s]+)(?:\s+\d|\s*$)',
                title_line + " " + body
            )
            if location_match:
                loc_candidate = location_match.group(1).strip()
                # Validate it looks like a location (not a company name)
                if not any(word in loc_candidate.lower() for word in ['inc', 'llc', 'corp', 'ltd', 'company']):
                    location = loc_candidate
        
        # Extract skills from the experience text using keyword matching
        extracted_skills = list(extract_skills_from_experience(full_text))
...
3. **Write Compelling Narratives** - Transform basic job duties into achievement stories with quantified impact where the candidate provided metrics
4. **Match Target Role** - Align language and focus with the job description requirements
5. **Be Truthful and Accurate** - Enhance and expand on what exists based on actual experience, but never fabricate accomplishments or metrics
6. **Use Provided Context Only** - Base all enhancements on the candidate's actual work history, extracted skills, and research insights
  ...
  "editorial_notes": "ATS-optimized"
    ...
    try:
        ...
        result = await call_llm_async(
            ...
            max_tokens=3000,  # Balanced for comprehensive output with cost control
            openrouter_api_keys=openrouter_api_keys,
            **kwargs
        )
...
async def run_analysis_async(
    ...
    **kwargs,
    ...
    if ENABLE_SEMANTIC_GAPS:
        ...
        # Extract job title from job_ad for context
        job_title_match = re.search(r'(?:position|role|title):\s*([^\n]+)', job_ad, re.IGNORECASE) if job_ad else None
        job_title = job_title_match.group(1).strip() if job_title_match else "this position"
        
        system_prompt = f"""
You are a gap analysis expert. Refine the heuristic gaps into structured, prioritized analysis for {job_title}.

CRITICAL: Every gap analysis must be SPECIFIC to this job's requirements. Reference actual responsibilities and skills from the job description.
...
{{
  "critical_gaps": [
    {{
      "skill": "Kubernetes",
      "severity": "high",
      "evidence": "Job requires container orchestration for crew analytics deployment",
      "impact": 0.9,
      "job_requirement": "Deploy models into production and monitor performance"
    }}
  ],
  "medium_gaps": [...],
  "nice_to_have_gaps": [...],
  "bridging_strategies": [
    {{
      "gap": "Kubernetes",
      "suggestion": "Led AWS ECS deployment serving 10k users - equivalent container orchestration experience for production ML systems",
      "job_context": "Translates to model deployment requirement"
    }}
  ],
  "overall_summary": "Resume strong in Python/AWS but needs to emphasize container orchestration experience for {job_title}'s production deployment requirements."
}}
        """
        user_prompt = f"""
...
Job description excerpt: {job_ad[:500] if job_ad else 'N/A'}

Analyze gaps SPECIFIC to this job. Every gap should reference actual job requirements.
        """
        try:
          llm_response = await call_llm_async(
              ...
              user_prompt,
              temperature=0.2,
              max_tokens=1200,  # Balanced for detailed gap analysis with cost control
    ...
    if not gap_analysis.get('critical_gaps') and job_ad and aggregate_skills:
        ...
        job_title_match = re.search(r'(?:position|role|title):\s*([^\n]+)', job_ad, re.IGNORECASE)
        job_title = job_title_match.group(1).strip() if job_title_match else "this position"
        
        # Extract keywords from job ad - now includes lowercase technical terms
        # Pattern matches: Capitalized words/phrases (Python), acronyms (AWS), and common lowercase tech terms
        job_keywords = set(re.findall(r'\b[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*\b|\b[A-Z]{2,}\b|\b(?:python|javascript|java|sql|docker|kubernetes|aws|azure|gcp|react|node|typescript|golang|rust|scala|kafka|redis|mongodb|postgres|mysql|git|linux|bash|html|css|api|rest|graphql|ci|cd)\b', job_ad, re.IGNORECASE))
        ...
        if inferred_gaps:
            ...
            gap_analysis["summary"] = f"For {job_title}, consider adding experience with: {', '.join(inferred_gaps)}. These skills are mentioned in the job requirements and would strengthen your application."
